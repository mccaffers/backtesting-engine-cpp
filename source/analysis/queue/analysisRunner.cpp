...
int AnalysisRunner::run(const std::string& questdbHost,
    ...
    // Launch the async task to process experiment runs
    asio::co_spawn(
        ioc,
        analysis_runner::drainExperiments(conn, questdbHost),
    ...
    if (error) {
        try {
            ...
            std::println(stderr, "AnalysisRunner failed: {}", ex.what());
            ...
            elastic::putEngineException(
                {elastic::nowIsoUtc(), "AnalysisRunner", ex.what(), ""});
        }
