...
Application::Application(std::unique_ptr<IStrategyFactory> factory) 
    : strategy(factory->createStrategy()),
      account(std::make_unique<Account>(10000.0)) {
    
    const char* filePath = std::getenv("DATA_FILE_PATH");
    if (filePath == nullptr) {
        throw std::runtime_error("DATA_FILE_PATH environment variable is not set.2");
    }
...
void Application::processTrading() {
    while (!csvStream->isFinished()) {
        ...
        if (!account->hasOpenTrade() && strategy->invoke(priceHistory)) {
